...
using RMS.Web.Areas.App.Models.CustomizableDashboard;
...
{
    public abstract class CustomizableDashboardControllerBase : RMSControllerBase
    {
        ...
        public async Task<PartialViewResult> AddWidgetModal(string dashboardName, string pageId)
        {
            var userDashboard = await DashboardCustomizationAppService.GetUserDashboard(
                new GetDashboardInput
                {
                    ...
                    Application = RMSDashboardCustomizationConsts.Applications.Mvc
                }
            );
