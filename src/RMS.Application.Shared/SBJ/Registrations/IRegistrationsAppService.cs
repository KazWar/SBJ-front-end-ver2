...
namespace RMS.SBJ.Registrations
{
	public interface IRegistrationsAppService : IApplicationService
	{
		...
		Task ComposeRegistrationStatusMessaging(long registrationId);

		Task<bool> SendFormData(string blobStorage, string blobContainer, FormRegistrationHandlingDto vueJsToRmsModel);

		Task<bool> UpdateFormData(string blobStorage, string blobContainer, UpdateRegistrationDataDto UpdateModel);

		Task<bool> BatAuthentication(FormRegistrationHandlingDto vueJsToRmsModel);
	}
}